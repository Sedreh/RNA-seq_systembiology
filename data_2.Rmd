...
# Let's make a new matrix de2_1 which will store information about pathways
...
# Let's look at all cancerous pathways

pathways.cancer <- m_df[m_df$gs_name %like% "cancer_", ]
pathways.cancer <- split(pathways.cancer$human_gene_symbol, pathways.cancer$gs_name)
...
fgseaRes_cancer <- fgsea(pathways=pathways.cancer, stats=ranks, nperm=1000)
...
ggplot(fgseaResTidy_cancer, aes(reorder(pathway, NES), NES)) +
  geom_col(aes(fill=pval<0.05)) +
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
       title="cancer pathways NES from GSEA") + 
  theme_minimal()
```
